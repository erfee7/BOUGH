...
async def test_create_and_fetch_message(db_transaction: asyncpg.Connection):
    ...
    conv_id = await create_conversation(title="Test", conn=db_transaction)
    msg_id = await create_message(conversation_id=conv_id, role="user", content="I need a Bough", conn=db_transaction)
    
    result = await fetch_message(message_id=msg_id, conn=db_transaction)
...
async def test_update_message(db_transaction: asyncpg.Connection):
    ...
    conv_id = await create_conversation(title="Test", conn=db_transaction)
    msg_id = await create_message(conversation_id=conv_id, role="assistant", content="", status="pending", conn=db_transaction)
    
    await update_message(msg_id, conn=db_transaction, content="Wonderful to see you", status="complete", metadata={"tokens": 5})
...
async def test_update_message_no_args(db_transaction: asyncpg.Connection):
    ...
    await update_message(msg_id, conn=db_transaction, invalid_col="ignore_me")
...
async def test_fetch_message_history_recursive(db_transaction: asyncpg.Connection):
    ...
    root_id = await create_message(conversation_id=conv_id, role="user", content="Root", conn=db_transaction)
    child_id = await create_message(conversation_id=conv_id, role="assistant", parent_id=root_id, content="Child", conn=db_transaction)
    grandchild_id = await create_message(conversation_id=conv_id, role="user", parent_id=child_id, content="Grandchild", conn=db_transaction)
...
async def test_fetch_conversation_messages_flat_list(db_transaction: asyncpg.Connection):
    ...
    await create_message(conversation_id=conv_id, role="user", content="Msg 1", conn=db_transaction)
    await create_message(conversation_id=conv_id, role="assistant", content="Msg 2", conn=db_transaction)
    
    messages = await fetch_conversation_messages(conversation_id=conv_id, conn=db_transaction)
...
async def test_delete_message(db_transaction: asyncpg.Connection):
    ...
    conv_id = await create_conversation(title="Test", conn=db_transaction)
    msg_id = await create_message(conversation_id=conv_id, role="user", content="Hello", conn=db_transaction)
    
    await delete_message(message_id=msg_id, conn=db_transaction)
    result = await fetch_message(message_id=msg_id, conn=db_transaction)
