...
async def update_conversation(conv_id: uuid.UUID, conn: asyncpg.Connection | None = None, **kwargs) -> None:
    """Updates a conversation. Pass columns to update as keyword arguments (e.g., title='New')."""
    if conn:
        return await _update_conversation(conn, conv_id, kwargs)
    
    pool = get_pool()
    async with pool.acquire() as conn:
        return await _update_conversation(conn, conv_id, kwargs)

async def _update_conversation(conn: asyncpg.Connection, conv_id: uuid.UUID, updates: dict) -> None:
    if not updates:
        logger.warning("update_conversation called with no columns to update for ID: %s", conv_id)
        return
    ...
    if not set_clauses:
        logger.warning("update_conversation called with no valid columns to update for ID: %s", conv_id)
        return
        
    args.append(conv_id)
    query = f"UPDATE conversations SET {', '.join(set_clauses)} WHERE id = ${idx};"
    await conn.execute(query, *args)
    logger.info("Updated conversation ID: %s with fields: %s", conv_id, list(updates.keys()))

async def delete_conversation(conv_id: uuid.UUID, conn: asyncpg.Connection | None = None) -> None:
    """Deletes a conversation and all its messages (via ON DELETE CASCADE)."""
    if conn:
        return await _delete_conversation(conn, conv_id)
    
    pool = get_pool()
    async with pool.acquire() as conn:
        return await _delete_conversation(conn, conv_id)

async def _delete_conversation(conn: asyncpg.Connection, conv_id: uuid.UUID) -> None:
    query = "DELETE FROM conversations WHERE id = $1;"
    await conn.execute(query, conv_id)
    logger.info("Deleted conversation with ID: %s", conv_id)